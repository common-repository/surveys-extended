...
$all_results = $wpdb->get_results($wpdb->prepare("SELECT ID, name, email FROM {$wpdb->prefix}surveys_extended_result WHERE survey_ID=%d ORDER BY added_on", $_REQUEST['survey']));
$questions = $wpdb->get_results($wpdb->prepare("SELECT ID, question FROM {$wpdb->prefix}surveys_extended_question WHERE survey_ID=%d", $_REQUEST['survey']));
...
$show_questions_in_header = get_option('surveys_extended_insert_csv_header');
