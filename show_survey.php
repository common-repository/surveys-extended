...
?>
<?php
if(!is_single() and isset($GLOBALS['surveys_extended_client_includes_loaded'])) { #If this is in the listing page - and a quiz is already shown, don't show another.
...
if(isset($_POST['action']) and $_POST['action']) { // Save the survey
	if($_POST['result_id']) { //Save the name and the email of the survey taker.
		$wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}surveys_extended_result SET name=%s, email=%s WHERE ID=%d", strip_tags($_POST['survey_taker_name']), strip_tags($_POST['email']), $_POST['result_id']));
		e("Thanks for taking the survey. Your details have been saved.");
		
	} else { //Save the survey details.
		//$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}surveys_extended_result (survey_ID, added_on) VALUES(%d, DATE_ADD(NOW(), INTERVAL %f HOUR))", $survey_id, get_option('gmt_offset')));
		$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}surveys_extended_result (survey_ID, added_on) VALUES(%d, NOW())", $survey_id));
		$result_id = $wpdb->insert_id;

		$question_count = 0;
		foreach($_POST['question_id'] as $question_id) {
			if(!$_POST['answer-' . $question_id]) { //User ignored the question.
				$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}surveys_extended_result_answer (result_ID, answer_ID, question_ID) VALUES(%d, %d, %d)", 
				...
				$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}surveys_extended_result_answer (result_ID, answer_ID, question_ID, user_answer) VALUES(%d, %d, %d, %s)", 
		...
		if($email) {
			...
			$questions = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}surveys_extended_question WHERE survey_ID=%d", $survey_id));
			...
			foreach($questions as $q) {
				...
				$all_answers_for_question = $wpdb->get_results($wpdb->prepare("SELECT A.answer,RA.answer_ID,RA.user_answer 
						FROM {$wpdb->prefix}surveys_extended_result_answer AS RA 
						LEFT JOIN {$wpdb->prefix}surveys_extended_answer AS A 
...
<?php
		$GLOBALS['surveys_extended_client_includes_loaded'] = true; // Make sure that this code is not loaded more than once.
	}

if($question) {
$questions_per_page = get_option('surveys_extended_questions_per_page');
...
<div id="survey-navigation">
    <table>
        <tr>
            <td>
                <input type="button" id="survey-previous-question" value="&lt; <?php e("Previous", 'surveyse') ?>"  />
            </td>
            <td>
                <input type="button" id="survey-next-question" value="<?php e("Next", 'surveyse') ?> &gt;"  />
            </td>
            <td class="table-row-filler">
            </td>
            <td>
...
foreach ($question as $ques) {
	...
	$all_answers = $wpdb->get_results("SELECT ID,answer FROM {$wpdb->prefix}surveys_extended_answer WHERE question_id={$ques->ID} ORDER BY sort_order");
...
/**
<!-- ASD change -->
<!-- input type="button" id="survey-previous-question" value="&lt; <?php e("Previous") ?>"  /-->
