...
$survey = $wpdb->get_row("SELECT S.ID, S.name, R.name AS rname, R.email, R.added_on FROM {$wpdb->prefix}surveys_extended_survey AS S INNER JOIN {$wpdb->prefix}surveys_extended_result AS R ON S.ID=R.survey_ID WHERE R.ID=$result_id");
...
$questions = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}surveys_extended_question WHERE survey_ID={$survey->ID}");
...
<ul>
<li><a href='edit.php?page=surveys-extended/individual_responses.php&amp;survey=<?=$survey->ID?>'>Individual Responses</a></li>
<li><a href='edit.php?page=surveys-extended/responses.php&amp;survey=<?=$survey->ID?>'>All Responses</a></li>
<li><a href='edit.php?page=surveys-extended/survey.php'>All Surveys</a></li>
</ul>
</div>