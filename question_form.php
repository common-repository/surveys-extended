...
$answer = array();
if($action == 'edit') {
	$answer		= $wpdb->get_results($wpdb->prepare("SELECT ID, answer FROM {$wpdb->prefix}surveys_extended_answer WHERE question_ID=%d ORDER BY sort_order", $_REQUEST['question']));
	$question	= $wpdb->get_row($wpdb->prepare("SELECT question,allow_user_answer,allow_multiple_answers,user_answer_format FROM {$wpdb->prefix}surveys_extended_question WHERE ID=%d", $_REQUEST['question']));
}
...
<form name="post" action="edit.php?page=surveys-extended/question.php" method="post" id="post">
<div id="poststuff">
...
<!-- Fix for qTranslate integration -->
...
<option value="entry" <?php if($question->user_answer_format == 'entry') echo 'selected="selected"'; ?>>Text Entry</option>
<option value="textarea" <?php if($question->user_answer_format == 'textarea') echo 'selected="selected"'; ?>>Textarea</option>
<option value="checkbox" <?php if($question->user_answer_format == 'checkbox') echo 'selected="selected"'; ?>>CheckBox</option>
</select>
</div>
...
<input type="hidden" name="question" value="<?=stripslashes($_REQUEST['question'])?>" />
<input type="hidden" id="user-id" name="user_ID" value="<?php echo (int) $user_ID ?>" />
<input type="hidden" name="action" value="<?php echo $action ?>" /> 
<span id="autosave"></span>
<input type="submit" name="submit" value="<?php e('Save') ?>" style="font-weight: bold;" tabindex="4" />
</p>
<a href="edit.php?page=surveys-extended/question.php&amp;survey=<?=$_REQUEST['survey']?>"><?php e("Go to Questions Page") ?></a>
</div>
</form>
