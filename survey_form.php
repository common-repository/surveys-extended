...
if($action == 'edit') {
	$survey_details = $wpdb->get_row("SELECT name,description,status FROM {$wpdb->prefix}surveys_extended_survey WHERE ID=$_REQUEST[survey]");
}
?>

<div class="wrap">
<h2><?php echo t(ucfirst($action) . " Survey"); ?></h2>
...
<div class="inside">
<label for="status">Active</label> <input type="checkbox" name="status" value="1" id="status" <?php if($survey_details->status or $action=='new') print " checked='checked'"; ?> />
</div></div>

</div>

<p class="submit">
<?php wp_nonce_field('surveys_extended_create_edit_survey'); ?>
