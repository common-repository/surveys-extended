...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']) {
	...
	foreach($options as $opt) {
		if(isset($_POST[$opt])) update_option('surveys_extended_' . $opt, $_POST[$opt]);
		else update_option('surveys_extended_' . $opt, 0);
	}
...
<input type="text" name="questions_per_page" value="<?php echo get_option('surveys_extended_questions_per_page') ?>" id="questions_per_page" size="3" /><br />

<label for="surveys_email"><?php e('Send all survey results by email') ?></label>
<input type="text" name="email" value="<?php echo get_option('surveys_extended_email') ?>" id="email" /><br />
...
?>
<input type="checkbox" name="<?=$option?>" value="1" id="<?=$option?>" <?php if(get_option('surveys_extended_'.$option)) print " checked='checked'"; ?> />
